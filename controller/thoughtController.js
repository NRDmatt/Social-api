...
deleteThought(req, res) {
   ...
   }).then(() => res.json({message: 'apps deleted!'})).catch((err) => res.status(500).json(err));
},
// add Reaction
addReaction(req, res) {
    ...
    Thought.findOneAndUpdate(
      ...
      .then((thought) =>
        !thought
          ? res
              .status(404)
              .json({ message: 'No friend found that ID' })
          : res.json(thought)
      )
      .catch((err) => res.status(500).json(err));
  },
    ...
    Thought.findOneAndUpdate(
      ...
      // console.log("get the deleteReaction")
        !thought
          ? res
              .status(404)
              .json({ message: 'No thought found with provided id' })
          : res.json(thought)
      )
      .catch((err) => res.status(500).json(err));
  },
